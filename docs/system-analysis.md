...
| `veo-3.1-fast-generate-001` | not configured — build-time | `us-central1` | Story-card memory clips, generated offline | n/a |
...
#### Generated imagery, and what it may not depict

The memory clip at the head of a story card is generated by Veo from what she
actually said. It shows **rooms, never people**: a plate of rice and a bottle of soy sauce
in a line house, a bare room above the coffee shop with the brazier smoking. The prompts say
so explicitly.

That is a product rule, not a stylistic one. Two inches below the image sits the quote box,
which is the one element the whole design exists to protect — her words, verbatim. Putting a
generated face beside it would place an invented person exactly where the product promises a
real memory, and a viewer has no way to tell which of the two is which. Rooms carry the
feeling without making that claim.

Assets are generated offline and committed (`web/public/memories/`, ~2.5 MB). Nothing calls
Veo at request time: a story card must never wait on a video model, and a card that fails to
open because inference was slow is worse than one with no picture.
...
| **Model Armor** | The screening façade | `sanitizeUserPrompt` against a template. It does **not** detect anything itself: its SDP filter delegates to Sensitive Data Protection, and in advanced mode to *your* DLP templates. Currently only the SDP filter is enabled, so it is a forwarder — see R18 |
| **Sensitive Data Protection (DLP)** | Detects and de-identifies | Where the work happens. `CREDIT_CARD_NUMBER` plus a custom `BANK_ACCOUNT_NUMBER` regex, and a de-identify template that replaces both. The names `BANK_ACCOUNT_NUMBER` in a finding come from here, not from Model Armor |
...
| The same, entities seeded | Call 1 produced two `born_at` facts; call 2 added nothing | **Measured at rev 5, and it is not phrasing.** The extractor proposed the fact in **8 of 8** runs across both phrasings. What varies is whether `build_facts` keeps it: a denial-led correction survived **3/4** runs, an assertion-only one **4/4**. Fact extraction runs at `temperature=0.1` and refusals are silent by design, so the same sentence registers or vanishes run to run with nothing said either way. See R15 |

The rev-4 note here claimed the extractor takes assertions and not denials, and built recording
guidance on it. That was wrong: it proposes the fact either way. Phrasing a correction as a
plain positive claim is still marginally more reliable — 4/4 against 3/4 on one small sample —
but the dominant effect is variance at the refusal boundary, not grammar.
...
| D26 | Screening calls DLP directly by default; Model Armor is opt-in | Model Armor's SDP filter delegates to the same DLP templates, so for redaction it is a forwarder — and it brought a failure of its own: a 200 with `EXECUTION_SKIPPED` when its service agent could not read those templates, which reports success while protecting nothing. Called directly the caller is the service account and a permission problem is an exception, which can be noticed. Model Armor stays reachable because prompt injection and jailbreak have no DLP equivalent and apply here — the transcript becomes part of the Archivist's prompt. Both were measured against the same transcript: identical redaction, 507ms and 429ms |
...
| ~~R18~~ | Model Armor was enabled with only its SDP filter, which delegates to DLP — a hop and a failure mode with no added detection | — | **Closed at rev 9** (D26). DLP is now the default backend; Model Armor is opt-in via `SAMPAN_SCREEN_BACKEND=armor` for the filters it alone has |
