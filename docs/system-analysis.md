...
| R18 | Model Armor is enabled with only its SDP filter, which delegates to DLP — so it currently adds a hop and a failure mode without adding detection | The extra hop brought a real failure: a 200 response with `EXECUTION_SKIPPED` when the service agent lacked DLP permission, which the first parse read as a clean transcript. Calling DLP directly would remove both. Kept because the filters unique to Model Armor — prompt injection and jailbreak in particular — apply directly to this system: the transcript is untrusted text that goes on to become part of the Archivist's prompt | **Open decision, not an accepted cost.** Either turn those filters on or drop the façade; carrying it for the SDP filter alone is the one option with no argument for it |
